...
public class MainDraw implements IChartDraw<ICandle> {
    ...
    private Paint mSelectorTextPaint2 = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    public MainDraw(BaseKLineChartView view) {
        ...
        mSelectorTextPaint2.setColor(ContextCompat.getColor(context, R.color.chart_white40));
    ...
    private void drawSelector(BaseKLineChartView view, Canvas canvas) {
        ...
        float height = ViewUtil.Dp2Px(mContext, 134);
        float width = ViewUtil.Dp2Px(mContext, 108);
        //float width = 0;
        float left;
        ...
        List<String> titles = Arrays.asList("时间", "开", "高","低", "收", "涨跌额", "涨跌幅", "成交量");
        List<String> values = new ArrayList<>();
        values.add(view.getAdapter().getDate(index));
        values.add(point.getOpenPrice() + "");
        values.add(point.getHighPrice() + "");
        values.add(point.getLowPrice() + "");
        values.add(point.getClosePrice() + "");
        values.add(point.getClosePrice() + "");
        values.add(point.getClosePrice() + "");
        values.add(point.getClosePrice() + "");
       ...
       /* for (String s : strings) {
        ...
        for (int i = 0; i < titles.size(); i++) {
            float valueWidth = mSelectorTextPaint2.measureText(values.get(i));
            canvas.drawText(titles.get(i), left + padding, y, mSelectorTextPaint);
            canvas.drawText(values.get(i), left + width - valueWidth-padding, y, mSelectorTextPaint2);
    ...
    public void setSelector2TextSize(float textSize) {
        mSelectorTextPaint2.setTextSize(textSize);
    }
