...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private void init() {
        ...
        mSelectedCorner = ViewUtil.Dp2Px(getContext(), 7);
    }
    ...
    private void initRect() {
        if (isShowChild) {
            int mMainHeight = (int) (displayHeight * 0.59f);
    ...
    private void drawK(Canvas canvas) {
        ...
        if (isLongPress) {
            ...
            //k线图竖线,渐变
            mSelectedYLinePaint.setShader(new LinearGradient(x, mMainRect.top, x, mMainRect.bottom, new int[]{0x0D356DF0, 0x66356DF0}, null, Shader.TileMode.MIRROR));
            //不渐变
            //canvas.drawLine(x, mMainRect.top, x, mMainRect.bottom, mSelectedYLinePaint);
            canvas.drawLine(x, mMainRect.top, x, mVolRect.bottom, mSelectedYLinePaint);
            // k线图横线
            canvas.drawLine(-mTranslateX, y, -mTranslateX + mWidth / mScaleX, y, mSelectedXLinePaint);

            //标识点
            //canvas.drawCircle(x, y, 6, mSelectPointPaint);
            // 柱状图竖线
    ...
    private void drawText(Canvas canvas) {
        ...
        //最右边刻度padding
        int padding = ViewUtil.Dp2Px(getContext(), 4);
        //--------------画上方k线图的值-------------
        if (mMainDraw != null) {
            canvas.drawText(formatValue(mMainMaxValue), mWidth - calculateWidth(formatValue(mMainMaxValue)) - padding, baseLine + mMainRect.top, mTextPaint);
            canvas.drawText(formatValue(mMainMinValue), mWidth - calculateWidth(formatValue(mMainMinValue)) - padding, mMainRect.bottom - textHeight + baseLine, mTextPaint);
            ...
            for (int i = 1; i < mGridRows; i++) {
                ...
                canvas.drawText(text, mWidth - calculateWidth(text) - padding, fixTextY(rowSpace * i + mMainRect.top), mTextPaint);
            }
        }
        //--------------画中间子图的值-------------
        if (mVolDraw != null) {
            String volMax = mVolDraw.getValueFormatter().format(mVolMaxValue);
            String volMin = mVolDraw.getValueFormatter().format(mVolMinValue);
            canvas.drawText(volMax, mWidth - calculateWidth(volMax) - padding, mMainRect.bottom + baseLine, mTextPaint);
            canvas.drawText(volMin, mWidth - calculateWidth(volMin) - padding, mVolRect.bottom, mTextPaint);
        }
        //--------------画下方子图的值-------------
        if (mChildDraw != null) {
            canvas.drawText(mChildDraw.getValueFormatter().format(mChildMaxValue), mWidth - calculateWidth(formatValue(mChildMaxValue)) - padding, mVolRect.bottom + baseLine, mTextPaint);
            canvas.drawText(mChildDraw.getValueFormatter().format(mChildMinValue), mWidth - calculateWidth(formatValue(mChildMinValue)) - padding, mChildRect.bottom, mTextPaint);
        }
        //--------------画时间---------------------
        float columnSpace = mWidth / mGridColumns;
        float y;
        if (isShowChild) {
            y = mChildRect.bottom + baseLine + 10;
        } else {
            y = mVolRect.bottom + baseLine + 10;
        }
        ...
        if (isLongPress) {
            // 画Y值
            mTextPaint.setColor(getResources().getColor(R.color.chart_white));
            IKLine point = (IKLine) getItem(mSelectedIndex);
            float w1 = ViewUtil.Dp2Px(getContext(), 4);
            float w2 = ViewUtil.Dp2Px(getContext(), 2);
            ...
            if (translateXtoX(getX(mSelectedIndex)) < getChartWidth() / 2) {
                ...
                path.lineTo(textWidth + 2 * w1, y - r);
                path.close();
                canvas.drawRoundRect(x, y - r, textWidth + 2 * w1, y + r, mSelectedCorner, mSelectedCorner, mSelectPointPaint);
                ...
                path.lineTo(x + w2, y - r);
                path.close();
                canvas.drawRoundRect(x, y - r, mWidth - 8, y + r, mSelectedCorner, mSelectedCorner, mSelectPointPaint);
                //canvas.drawPath(path, mSelectPointPaint);
                //canvas.drawPath(path, mSelectorFramePaint);
                canvas.drawText(text, x + w1 + w2 - 6, fixTextY1(y), mTextPaint);
            }
            ...
            canvas.drawRoundRect(x - textWidth / 2 - w1, y, x + textWidth / 2 + w1, y + baseLine + r, mSelectedCorner, mSelectedCorner, mSelectPointPaint);
    ...
    public void setSelectedXLineColor(int color) {
        mSelectedXLinePaint.setColor(getResources().getColor(color));
    }
    ...
    public void setSelectedYLineColor(int color) {
        mSelectedYLinePaint.setColor(getResources().getColor(color));
    }
