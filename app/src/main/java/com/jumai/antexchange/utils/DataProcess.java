...
public class DataProcess {
    ...
    public static String keepFourDecimals(double var) {
        return mFourDecimalFormat.format(new BigDecimal(var));
    }

    /**
     * 将double数据格式化为指定小数位的String，不足小数位用0补全
     */
    public static String keepDecimals(double var, int scale) {
        if (scale <= 0) {
            return "- -";
        }

        StringBuilder decimals = new StringBuilder("0.");
        for (int i = 0; i < scale; i++) {
            decimals.append("0");
        }
        return new DecimalFormat(decimals.toString()).format(var);
    }
    ...
    public static String keepTwoDecimals(double var) {
        return mTwoDecimalFormat.format(new BigDecimal(var));
    }
     ...
     * 保留两位小数
     */
    public static String keepTwoDecimals(String var) {
        return mTwoDecimalFormat.format(new BigDecimal(var));
    }
    ...
    public static String formatRAMKB(long ramByte) {
        return keepFourDecimals(ramByte / 1024f);
    }
}
