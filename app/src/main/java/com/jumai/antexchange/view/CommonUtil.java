...
public class CommonUtil {
    ...
    public static int getScreenWidth() {
        if (mScreenWidth == -1) {
            DisplayMetrics displayMetrics = AntApplication.getInstance().getResources().getDisplayMetrics();
    ...
    public static boolean needUpgrade(Context context, String newVersionName) {
        String[] currVersion = getVersionName(context).split("\\.");
        String[] newVersion = newVersionName.replace("v", "").split("\\.");
        if (Integer.valueOf(newVersion[0]) > Integer.valueOf(currVersion[0])) {
            return true;
        }

        if (Integer.valueOf(newVersion[1]) > Integer.valueOf(currVersion[1])) {
            return true;
        }

        return Integer.valueOf(newVersion[2]) > Integer.valueOf(currVersion[2]);
    }
    ...
    public static String phoneHide(String phone) {
        ...
        String str2 = phone.substring(phone.length() - 4);
        return str1 + "****" + str2;
    }
    ...
    public static boolean launchApp(String packageName, Bundle data) {
        try {
            Intent resolveIntent = AntApplication.getInstance().getPackageManager().getLaunchIntentForPackage(packageName);
            if (resolveIntent == null) {
                PackageInfo p = AntApplication.getInstance().getPackageManager().getPackageInfo(packageName, PackageManager.GET_ACTIVITIES);
                if (p != null) {
            ...
            if (resolveIntent != null) {
                ...
                AntApplication.getInstance().startActivity(resolveIntent);
                return true;
            }
        } catch (Exception e) {
    ...
    public static void launchApp(Context context, String packageName, String activityName, Bundle data) {
        ...
        try {
            ...
            e.printStackTrace();
            ToastUtils.showShort(R.string.tips_app_not_found);
        }
    }
    ...
    public static void copyAnchor(String contentStr) {
        ClipboardManager clipBoard = (ClipboardManager) AntApplication.getInstance().getSystemService(Context.CLIPBOARD_SERVICE);
    ...
    public static int getNavigationBarHeight() {
        if (mNavigationBarHeight == -1) {
            Resources resources = AntApplication.getInstance().getResources();
    ...
    public static int getStatusHeight() {
        int statusHeight = 0;
        int resourceId = AntApplication.getInstance().getResources().getIdentifier("status_bar_height", "dimen", "android");
        if (resourceId > 0) {
            statusHeight = AntApplication.getInstance().getResources().getDimensionPixelSize(resourceId);
        }
        return statusHeight;
    }
    ...
    public static boolean isComponentEnable(String componentName) {
        int isEnabled = AntApplication.getInstance().getPackageManager().getComponentEnabledSetting(new ComponentName(AntApplication.getInstance().getPackageName(), componentName));
    ...
    public static int getComponentState(String componentName) {
        return AntApplication.getInstance().getPackageManager().getComponentEnabledSetting(new ComponentName(AntApplication.getInstance().getPackageName(), componentName));
    }
    ...
    public static void enableComponent(String componentName) {
        ...
        AntApplication.getInstance().getPackageManager().setComponentEnabledSetting(new ComponentName(AntApplication.getInstance().getPackageName(), componentName),
    ...
    //包含大小写和数字
    public static boolean loginPwdReg(String pwdCode) {
        return Pattern.matches("^[A-Za-z0-9]{6,12}$", pwdCode);
    }

    //包含大小写和数字
    public static boolean transactionPwdReg(String pwdCode) {
        return Pattern.matches("^[A-Za-z0-9]{6,8}$", pwdCode);
    }
}