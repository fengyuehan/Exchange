...
public class CoinListHeadDecoration extends RecyclerView.ItemDecoration {
    private final int mBgColor1;
    private int mBgColor;
    private int mTextColor;
    private Context mContext;
    private List<HotCoinBean> mData;
    ...
    public CoinListHeadDecoration(Context context, List<HotCoinBean> data) {
        super();
        mContext = context;
        mData = data;
        mPaint = new Paint();
        ...
        mRight = CommonUtil.dp2px(context, 45);
        int textSize = CommonUtil.sp2px(context, 16);
        mPaint.setTextSize(textSize);
        mPaint.setAntiAlias(true);
        mBgColor = context.getResources().getColor(R.color.white);
        mBgColor1 = context.getResources().getColor(R.color.color_f7f8fa);
        mTextColor = context.getResources().getColor(R.color.tab_selected);
    }
    ...
    public void onDraw(@NonNull Canvas canvas, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {
                    drawHead(canvas, left, right,child, recyclerView, params, position);
                } else {
                    if (!mData.get(position).getName_first_letter().equals(mData.get(position - 1).getName_first_letter())) {
     ...
     * 绘制头部区域背景和文字
     */
    private void drawHead(Canvas canvas, int left, int right, View child,RecyclerView parent, RecyclerView.LayoutParams params, int position) {
        ...
        canvas.drawRect(0, parent.getPaddingTop(), parent.getRight() - parent.getPaddingRight()- mRight, parent.getPaddingTop() + mHeadHeight, mPaint);
        mPaint.setColor(mTextColor);

        //将文本放入到一个矩形框中，并测量文本的宽和高

        String text = mData.get(position).getName_first_letter();
        if (text != null){
            if (text.length() == 2) {
            ...
            canvas.drawText(text, CommonUtil.dp2px(mContext, 15), baseLine, mPaint);
        }
    }
    ...
    public void onDrawOver(@NonNull Canvas canvas, @NonNull final RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        if ((position + 1) < mData.size()) {
            ...
            if (!text.equals(mData.get(position + 1).getName_first_letter())) {
